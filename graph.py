    ...
    while len(stack) > 0:
        ...
        for inst in dis:
            ...
            if mne in ['call', 'jmp']:
                ...
                # Terminate the search.
                break
    #print(f"Leaders are: {leaders}")
    return leaders


def graph(rad, leaders):
    dot = Digraph()
    leader_list = list(leaders)
    leader_list.sort(reverse=True)
    empty=[]

    def range_check(leader):
        if rad.is_in_range(leader):
            return True
        else:
            return False
    ...
    while len(leader_list) > 0:
        leader = leader_list.pop()
        dot.node(str(hex(leader)))
        if range_check(leader)==True:
            empty.append(hex(leader))
        dis = rad.dis(leader)
        for inst in dis:
            ...
            if mne in ['call','jmp']:
                ...
                if target.type == X86_OP_IMM and range_check(target.value.imm)==True:
                    empty.append(hex(target.value.imm))
                    #dot.node(str(hex(target.value.imm)))
                    dot.edge(str(hex(leader)), str(hex(target.value.imm)))
                if mne == 'jmp':
                    break
            elif mne =='loop' or mne.startswith('j'):
                target = inst.operands[0]
                if target.type == X86_OP_IMM:
                    if range_check(target.value.imm)==True:
                        dot.edge(str(hex(leader)), str(hex(target.value.imm)))
                        empty.append(hex(target.value.imm))
                    if range_check(rip)==True:
                        empty.append(hex(rip))
                        dot.edge(str(hex(leader)), str(hex(rip)))
                    #dot.node(target.value.imm)
                    #dot.node(rip)
                    #dot.edge(leader, target.value.imm)
                    #dot.edge(leader,rip)
                break
            if mne in ['ret','reti','hlt']:
                break
    return dot
