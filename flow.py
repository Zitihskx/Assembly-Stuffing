...
def pass_two(rad: RAD, leaders: Set[int]) -> None:
    ...
    for leader in leads:
        ...
        for inst in block:
            ...
            last_mne = inst.mnemonic
            last_operands = inst.operands
            last_rip = inst.address + len(inst.bytes)

        if last_mne in ['jmp']:
            target = last_operands[0]
            for inst in block:
                print(f'0x{inst.address:x}:    {inst.mnemonic} {inst.op_str}')
            if target.type==X86_OP_IMM:
                print(f'L := 0x{target.value.imm:x}')
        elif last_mne=='loop' or last_mne.startswith('j'):
            target = inst.operands[0]
            print('if')
            for inst in block:
                print(f'  0x{inst.address:x}:  {inst.mnemonic}  {inst.op_str}')
